...
# DREAM-CS-Fast DDP runner on GPU 6 and GPU 7.
# This trains one model with two processes, not two independent seeds.
# Default FAST_MODE=bank_plus_anchor is the no-regret-safe main candidate.
# Set FAST_MODE=single_bank only for speed-first ablation/debug.
...
MODEL_NAME=${MODEL_NAME:-dream_cs_fast_${FAST_MODE}_ufd_ddp_gpus67_seed${SEED}}
LOG_FILE=${LOG_FILE:-dream_cs_fast_${FAST_MODE}_ddp_gpus67_seed${SEED}.log}
STATUS_FILE=${STATUS_FILE:-/tmp/dream_cs_fast_${FAST_MODE}_ddp_gpus67_seed${SEED}.status}

echo "Starting one DREAM-CS-Fast DDP job in the foreground:"
echo "  GPUs=${GPU_IDS} nproc=${NPROC_PER_NODE} seed=${SEED}"
echo "  FAST_MODE=${FAST_MODE} model=${MODEL_NAME}"
echo "  output=outputs/${MODEL_NAME}"
echo "  log=${LOG_FILE}"
echo "  per-GPU batchsize=${BATCHSIZE}; effective train batch is approximately $((BATCHSIZE * NPROC_PER_NODE))"
echo "  Console output is mirrored to the log with the same [TRAIN] epoch/iter format."
